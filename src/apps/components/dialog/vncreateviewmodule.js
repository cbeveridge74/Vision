...
.factory('vnCreateViewFactory', function( 
	$rootScope, 
	$timeout, 
	vnTaskCreateFactory,
	vnWorkflowFactory,
	...
	factory.sendTask = function( isTask, callback, suppressToasts ){
		var model = factory.getModel();
		var attachments = {};
		attachments.attachedDocs = model.documentCollection;
		attachments.selectedPatients = model.patientCollection;
		...
		if( isTask ){
			...
			attachments.trackingSummary = model.trackingSummary;
			attachments.recurrenceSummary = model.recurrenceSummary;
			vnWorkflowFactory.convertTaskToWorkflow( task, attachments, function( workflowedTask ){
				vnWorkflowFactory.saveWorkflow ( workflowedTask, function(){
					vnWorkflowFactory.beginWorkflow( workflowedTask.name, suppressToasts );
					if( callback != null ){
						callback();
					}
				});
	...
	$rootScope.$on( vnTaskCreateFactory.TASK_CREATE_UPDATED, function( event ){
		var model = factory.getModel();
		model.documentCollection = vnTaskCreateFactory.getModel().documentCollection;
		model.patientCollection = vnTaskCreateFactory.getModel().patientCollection;
		model.selectedSubjects = vnTaskCreateFactory.getModel().selectedSubjects;
		model.selectedRecipients = vnTaskCreateFactory.getModel().selectedRecipients;
		model.description = vnTaskCreateFactory.getModel().description;
		$timeout( function(){
    ...
    return {
      ...
      controller:  function ( 
			...
			VN_MAIL_SCREENS
		  	...
		  	$rootScope.$on( vnScreenManagementFactory.GENERAL_CLICK, function( event ){
	  			if( ( $rootScope.screens.VN_REP__HOME == true 
	  				|| $rootScope.screens.VN_TASK == true
	  				|| $rootScope.screens.VN_MAIL == true) && $( "." + $scope.classIdentifier + '.create-view .md-dialog-container' ).length < 1 ){

	  				// Okay GLOBAL_COUNT_FOR_SHOWING_VN_CREATE_VIEW_CONFIRMATION_DIALOG is a big hack.  There a multiple instances of this directive dotted around the place
	  				// and I only want to show this confirmation dialog once.
	  				if( $( "." + $scope.classIdentifier + ".menu.create-view" ).is(':visible') && ++GLOBAL_COUNT_FOR_SHOWING_VN_CREATE_VIEW_CONFIRMATION_DIALOG == 1 ){
	  					var confirm = $mdDialog.confirm()
				          
				          .content('This will close the task without sending it.')
				          .ariaLabel('Warning')
				          //.targetEvent(ev)
				          .ok('Close without sending')
				          .cancel("Don't close");
				        ...
				        $mdDialog.show(confirm).then(function() {
					      ...
					      GLOBAL_COUNT_FOR_SHOWING_VN_CREATE_VIEW_CONFIRMATION_DIALOG = 0;
					    });
	  				}
		  		}
			...
			$scope.handleSaveClick = function( event ){
				...
				/*
				var attachments = {};
				attachments.documentCollection = $scope.model.documentCollection;
				...
				*/
				vnCreateViewFactory.sendTask( $scope.isTask, function(){
					closeDialog();
				});
			};
