...
.factory('vnDatabaseFactory', function( 
  ...
  $rootScope.$watch( "dbloaded", function( value ){
    if( value ){
      ...
      angular.forEach( calendarQueue, function( value, index ){
        factory.retrieveSlotDataForCalendar( value.callback );
      });
  ...
  factory.open = function( successHandler ) {
      ...
      request.onupgradeneeded = function(e) {
        ...
        angular.forEach( database, function( value, key ){
          var store;
          var autoincrement     = value.index[0].autoincrement;
          var compound          = value.index[0].compound;
          var unique            = value.index[0].unique;
          ...
          if( autoincrement == true ){
            ...
            store = db.createObjectStore(value.name,{keyPath: value.index[0].attributes}, { unique:unique });
          }else{
          ...
          angular.forEach( value.index, function( indexValue, indexKey ){
            ...
            if( multi ){
              ...
              store.createIndex(indexValue.name, indexValue.attributes, {unique:uniquei, multiEntry:multi});
            }else{
  ...
  var loadMailMessages = function(){
    var fileNumber = 237;
    factory.retrieveData( database[ MAIL_ITEMS ].name, function( results ){
      results.forEach( function( result, resultIndex ){
        $http.get('data/mailmessages/TEST' + fileNumber++ + '.xml').
        then(function(response) {
            var localResult = result;//response.config.data.result;
            
            
            var xml = response.data;
            xml = xml.replace( /xmlns="##other"/g, "" );
            var json = $.xml2json( xml, true );
            console.log( json );
            var reportData = json.ReportData[0];
            var serviceResult = reportData.ServiceResult[0];
            var testResults = serviceResult.TestResultSets[0].TestResultSet[0].TestResults[0].TestResult;
            var meta = {};

            meta.reportdate = reportData.ReportDate[0].text;
            meta.reportid = reportData.ReportIdentifier[0].text;

            if( localResult.type.displayname != "New scanned doc" ){
              localResult.type.displayname = json.ReportData[0].Discipline[0].text;
            }
            // DCB uncomment this to use the actual dates from the test messages
            //localResult.datereceived = new Date( moment( serviceResult.SampleDetails[0].DateTimeReceived[0].text ) );
            testResults.forEach( function( testResult ){
              if( localResult.results == null ){
                localResult.results = [];
              }
              /*if( testResult.TestPerformed == null ){
                testResult = testResult.TestResults[0].TestResult[0];
              }*/
              var unit = ""; 
              var value = ""
              var limits = {};
              if( testResult.TestMeasurement == null ||
                testResult.TestMeasurement[0].MeasurementNumeric == null || 
                testResult.TestMeasurement[0].MeasurementNumeric[0].Result[0].QuantityUnit == null ){
                
              }else{
                unit = testResult.TestMeasurement[0].MeasurementNumeric[0].Result[0].QuantityUnit[0].text;
                value = testResult.TestMeasurement[0].MeasurementNumeric[0].Result[0].QuantityValue[0].text;
                if( testResult.TestMeasurement[0].ReferenceLimit[0].ReferenceLimitNumeric != null ){
                  limits.high = testResult.TestMeasurement[0].ReferenceLimit[0].ReferenceLimitNumeric[0].RangeHighValue[0].text;
                  limits.low = testResult.TestMeasurement[0].ReferenceLimit[0].ReferenceLimitNumeric[0].RangeLowValue[0].text;
                }

                
              }
              localResult.meta = meta;

              localResult.results.push({
                testid: testResult.TestPerformed[0].TestName[0].ClinicalInformation[0].ClinicalCode[0].ClinicalCodeValue[0].text,
                testname: testResult.TestPerformed[0].TestName[0].ClinicalInformation[0].ClinicalCodeDescription[0].text,
                testvalue: value,
                testunits: unit,
                testlimits: limits
              });
            ...
            factory.updateData( database[ MAIL_ITEMS ].name, localResult, function(){
              

              if( resultIndex == results.length - 1 ){
                $rootScope.$broadcast( "MAIL_MESSAGES_LOADED" );
              }
            });
        }, function(response) {
  ...
  factory.retrieveSlotDataForCalendar = function( callback){
  ...
  factory.updateData = function( item, data, callback ) {
    //DCB note: If you're updating existing data ensure that the default key is first in the
    // config list (e.g. see TASKS)
