...
.controller('vnTitleBarController', function( 
  ...
  $scope.init = function(){
    ...
    if( $scope.backbuttonimg == null ){
      $scope.backbuttonimg = "images/left_arrow_blue.svg";
    }
  };

  $scope.handleInformCommandClick = function( e ){
    $rootScope.$broadcast( "INFORM_COMMAND_SELECTED", this.informCommand, e );
    gaTracker.sendEvent('Inform Command', this.informCommand.id, 'Inform commands');
  };
  ...
  function handleSelected (event, selected){
      ...
      gaTracker.sendEvent('Main patient search', "Patient selected");
...
.directive('vnTitleBar', function() {
    return {
      ...
      scope: {
        ...
        backbuttonimg: '@vnBackButtonImg',
        showsearch: '=vnShowSearch',
        showcommands: '=vnShowCommands',
        commands: '=vnCommands',
