...
ValidationUtil.isDisabled = function( fieldId ){
...
ValidationUtil.compareDates = function( dateField1, dateField2 ){
	var date1 = $( dateField1 ).attr( 'value' );
	var date2 = $( dateField2 ).attr( 'value' );
			...
			_isConditionTrue: function( val, rule ){
				...
				switch(rule.operator){
				    ...
				    case 'ends-with':
				        break;
				    case 'disabled':
				    	ret = ValidationUtil.isDisabled( rule.selector );
				        break;
				    case 'not-disabled':
				    	ret = !ValidationUtil.isDisabled( rule.selector );
				        break;
				    ...
				    case 'greater-than':
				    	if( $( rule.selector ).hasClass( 'hasDatepicker' ) ){
				    		if( rule.value == 'DOB' ){
				    			ret = ValidationUtil.compareDates( rule.selector, '#inps_patientdateofbirth' ) > 0;
				    			break;
				    		}else if( rule.value == 'DOD' ){
				    			ret = ValidationUtil.compareDates( rule.selector, '#inps_patientdateofdeath' ) > 0;
				    			break;
				    		}else if( rule.value == 'TODAY' ){
				    			ret = ValidationUtil.compareDates( rule.selector, '#inps_systemdate' ) > 0;
				    			break;
				    		}
				    		ret = ValidationUtil.compareDates( rule.selector, rule.value ) > 0;
