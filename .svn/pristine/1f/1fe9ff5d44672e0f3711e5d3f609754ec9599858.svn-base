	...
	if ( bits.length == 0 && token.length < 2 ) {
		...
		suggestions.sort(function(a,b){
			...
			return 0;
				...
				if ( self.words[wordId].length >= 4 && self.words[wordId].replace(/\d+/g,'') == self.words[wordId] ){
		...
		callback({words:result, queries: self.suggestFromHistory(entered, limit)});
		...
		console.log('Processed ',  result)
		callback({words:result, queries:suggestFromHistory(entered, limit)});
	}
}

var recordInHistory = function ( query, previousChoices, group ) {
	// see if already have this query
	var trimmedQuery = query.trim();
	if ( typeof( self.history[trimmedQuery] ) != 'undefined' ) {
		self.history[trimmedQuery].uses++;
		var choices = {};
		self.history[trimmedQuery].previousChoices.forEach(function(choice){
			choices[choice.wordId] = true;
		})
		previousChoices.forEach(function(choice){
			choices[choice.wordId] = true;
		});
		self.history[trimmedQuery].previousChoices = [];
		Object.keys(choices).forEach(function(choice){
			self.history[trimmedQuery].previousChoices.push({wordId:choice})
		})
	} else {
		self.history[trimmedQuery] = {uses: 1, query: query, previousChoices: previousChoices, group:group }
	}
	console.log(self.history);
}

var suggestFromHistory = function ( query, limit ) {
	var suggestions = [];
	var trimmedQuery = query.trim();
	console.log('Query:', query);
	Object.keys(self.history).sort(function(a,b){
		if ( self.history[a].uses > self.history[b].uses ) return -1;
		if ( self.history[a].uses < self.history[b].uses ) return 1;
		if ( a > b ) return 1;
		if ( a < b ) return -1;
		return 0;
	}).some(function(history){
		if ( self.history[history].query.trim().substring(0,query.length) == trimmedQuery ) {
			suggestions.push(self.history[history]);
...
var find = function ( query, previousChoices, group, limit, callback ){
	recordInHistory(query, previousChoices, group);
	var results  = {};
	var findTokens = 0;
