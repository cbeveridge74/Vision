...
.factory('vnWorkflowFactory',  function( 
		...
		visionFactory.retrieveData( database[ TASK_WORKFLOW ].name, function( workflow ){
			...
			angular.forEach( workflow[0].tasks, function( outerTaskValue, outerTaskIndex ){
				...
				angular.forEach( outerTaskValue, function( innerTaskValue, innerTaskIndex ){
					var due;
					if( innerTaskValue.due != null ){
						due = moment( innerTaskValue.due ).toDate();
					}
	...
	factory.beginWorkflow = function( name, suppressToasts ){
		...
		factory.retrieveWorkflow( name, function( workflow ){
			workflow = workflow[0];
			workflow.started = new Date();
			visionFactory.updateData( database[ TASK_WORKFLOW ].name, workflow, function(){
				var showToast = false;
				var firstSetOfTasks = workflow.tasks[0];
	...
	factory.convertTaskToWorkflow = function( task, attachments, callback ){
		...
		if( attachments != null ){
			localAttachments.attachedDocs 		= attachments.attachedDocs 		? attachments.attachedDocs : [];
			localAttachments.recurrenceSummary 	= attachments.recurrenceSummary ? attachments.recurrenceSummary : {};
			localAttachments.selectedPatients 	= attachments.selectedPatients 	? attachments.selectedPatients : [];
			localAttachments.trackingSummary 	= attachments.trackingSummary 	? attachments.trackingSummary : {};
		}
	...
	factory.retrieveWorkflowsFor = function( assigner, callback ){
		var bounds = {start: assigner, end: assigner};
		var index = 'by_assigner';
		if( assigner == null ){
			bounds = null;
			index = null;
		}
		...
		visionFactory.retrieveData( database[ TASK_WORKFLOW ].name, function( workflows ){
			angular.forEach( workflows, function( vWorkflow, iWorkflow ){
				//DCB Just using the raw file here
				vWorkflow.assigner = new DataStore().demoDataPersons[ vWorkflow.assigner ];
			});
			if( callback != null ){
				callback( workflows );
			}
      ...
      controller: function( $scope, 
      	...
      	var handleShow = function(){
      		...
      		if( vnWorkflowFactory.CURRENT_WORKFLOW == null ){
      			...
      			$scope.workflowOwner = vnWorkflowFactory.CURRENT_WORKFLOW.tasks[0][0].assigner.shortlabel;
      		}
      		$timeout( function(){
      			handleResize();
      		},100 );
      	...
      	$rootScope.$on( "INFORM_COMMAND_SELECTED", function( scope, value, e ){
      		if( value.id == WRKSND || value.id == WRKSVE ){
      			handleInformCommandSelected( scope, value.id, e );
      		}
      	});
		...
		$scope.handleOuterAddClick = function(){
			gaTracker.sendEvent('Workflow', 'Create Series Task', 'Series');
			vnWorkflowFactory.addCardContainer();
			handleResize();
			$timeout( function(){
		...
		var handleResize = function(){
			...
			//var iscroll = $( ".vision-workflow .iscroll" );
			//iscroll.width( iscroll.width() + 10);
