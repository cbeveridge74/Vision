...
( function () {
  ...
  function configureXaxis( chart, scope, attrs ) {
    ...
    if ( attrs.xaxistickformat ) {
      ...
      //chart.xAxis.xTickFormat( scope.xaxistickformat() );
    }
    if ( attrs.xaxislabel ) {
            ...
            if ( data ) {
              ...
              nv.addGraph( {
                generate: function () {
                  ...
                  var chart = nv.models.lineWithFocusChart().width( scope.width ).height( scope.height ).focusHeight( attrs.height2 === undefined ? 100 : +attrs.height2 ).margin( scope.margin ).focusMargin( scope.margin2 ).x( attrs.x === undefined ? function ( d ) {
                    return d[ 0 ];
                  } : scope.x() ).y( attrs.y === undefined ? function ( d ) {
                    return d[ 1 ];
                    ...
                    return attrs.isarea === 'true';
                  } ).interactive( attrs.interactive === undefined ? false : attrs.interactive === 'true' ).interpolate( attrs.interpolate === undefined ? 'linear' : attrs.interpolate );
                  if ( attrs.defined ) {
              ...
              nv.addGraph( {
                generate: function () {
                  ...
                  var chart = nv.models.sparklinePlus().width( scope.width ).height( scope.height ).margin( scope.margin ).x( attrs.x === undefined ? function ( d ) {
                    ...
                    return d.y;
                  } : scope.y() ).color( attrs.color === undefined ? nv.utils.getColor( [ '#000' ] ) : scope.color() ).alignValue( attrs.alignvalue === undefined ? true : attrs.alignvalue === 'true' ).rightAlignValue( attrs.rightalignvalue === undefined ? false : attrs.rightalignvalue === 'true' ).noData( attrs.nodata === undefined ? 'No Data Available.' : scope.nodata );
                  if ( attrs.xScale ) {
                    chart.xScale( scope.xScale() );
                  }
                  if ( attrs.yScale ) {
