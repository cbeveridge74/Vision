function DemoAssembler( $http ){
	...
	var BLOOD = dataStore.mailType.BLOOD;
	var SERUM = dataStore.mailType.SERUM;
	var SCAN = dataStore.mailType.SCAN;
	var MAIL_TYPES = [];
	MAIL_TYPES[ BLOOD ] = { id: BLOOD, displayname: "Blood" };
	MAIL_TYPES[ SERUM ] = { id: SERUM, displayname: "Serum" };
	MAIL_TYPES[ SCAN ] = { id: SCAN, displayname: "New scanned doc" };

	var filterCategories = new DataStore().filterCategories;
	var MAIL_CATEGORIES = [];
	MAIL_CATEGORIES[ filterCategories.TESTS_INVEST ] = [ MAIL_TYPES[ BLOOD ] ];
	MAIL_CATEGORIES[ filterCategories.HOSPITAL_REPORTS ] = [ MAIL_TYPES[ SERUM ] ];

	assembler.assembleMailItemOrder = function( data ){
		return data;
	};

	assembler.assembleMailFilters = function( data ){

		var users = new DataStore().demoDataPersons;
		var returnData = [];
	...
	assembler.assembleMailItem2015 = function( data ){
		var demoData = new DataStore();
		
		data.patient = demoData.patients[ data.patientid - 1 ];
		data.user = demoData.demoDataPersons[ data.userid ];
		data.type = MAIL_TYPES[ data.typeid ];

		MAIL_CATEGORIES.forEach( function( category, categoryIndex ){
	...
	assembler.assembleMailItems = function( data ){
		
		var returnData =  dataAssembler( data, assembler.assembleMailItem2015 );
		return returnData;
	};


	assembler.assembleNotifications = function( data ){
		return data;
	};

	assembler.assembleComments = function( data ){
		return data;
	};
