...
.controller('vnDetailViewController', function ( 
	...
	var initialiseActions = function(){
		...
		vnTaskActionFactory.initialiseActions( vnDetailViewFactory.getModel(), function( leftActions, actions ){
			$scope.leftActions = leftActions;
	...
	$scope.handleGeneralClick = function( event ){
	    ...
	    if( snoozeOpen ){
	    	vnScreenManagementFactory.closeMenu( ".vision-snooze" );
	    	snoozeOpen = false;
	    }
	...
	$scope.handleActionClick = function( e ){
	...
	$scope.handleDocumentClick = function(){
		...
		if( doc.type == new DataStore().docType.MAIL_MANAGER ){
			vnActionFactory.needHelpWith( vnActionFactory.MAIL_ITEM_DISPLAY, [ "1st Arg" ], function(){
				console.log( "I've been helped" );
			} );
		}
	};
