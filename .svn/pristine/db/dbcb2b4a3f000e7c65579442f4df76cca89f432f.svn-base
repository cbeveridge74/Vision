	...
	.directive('mdChips', ['$compile','$timeout', '$document', 'chipsService', function($compile, $timeout, $document, chipsService){
		return {
			...
			template: '<div class="md-chips" ng-cloak> \
							<div class="chips-input-field"> \
								<div class="input-chips-elements"> \
									...
									<input type="text" placeholder="{{vnPlaceholderString}}" ng-model="chipsText[mdTitle]" ng-focus="clearActive()" ng-keydown="clearPrev($event)" class="chipsInput"/> \
								</div> \
							</div> \
						</div>',
			require: '?ngModel',
			...
			link: function (scope, element, attrs) {
				scope.ytop = '10px';
				scope.vnPlaceholderString = attrs.vnPlaceholder;
				// DCB Added
				scope.$watch( 'ngModel', function( newValue, oldValue ) {
					...
					if( newValue == null || newValue.length < 1 ){
						...
						scope.vnPlaceholderString = attrs.vnSecondaryPlaceholder;
					}
				}, true);

				scope.$watch( 'collection', function( newValue, oldValue ) {
					if( scope.collection == null || ( newValue.length == oldValue.length ) ){
						return;
					}
			   ...
			   }, true );// DCB Needs to be deep watch
				...
				scope.addToInput = function(item){
					...
					this.removeList();
					element[0].querySelector('.chipsInput').focus();
					if( scope.addChipsHandler != null ){
						scope.addChipsHandler();
					}
				};
				...
				scope.showMore = function(index, event){
					...
					// DCB Add a template and this will inject the id into it
					if( scope.vnTemplate != null && scope.vnTemplate.length > 0){
						var temp = scope.vnTemplate;
						temp = '<' + temp + ' id="' + item.id + '"></' + temp + '>';
						htmlCode = temp;
					}
